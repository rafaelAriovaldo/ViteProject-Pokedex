...
export function TeamMain() {
  ...
  async function createNewTeam(event: FormEvent) {
    ...
    const TeamData = {
      ...
      pokemons: formData.getAll("pokemon"),
    };
    ...
    <div id="my form">
      <form onSubmit={createNewTeam}>
        <fieldset className="mt-5">
          <label htmlFor="teamName" className="ml-[45rem]">
            Nome do Time:
            <input
              name="teamName"
              id="teamName"
              type="text"
              className="  ml-1 border-red-800 items-center bg-slate-50 rounded-lg"
              placeholder="nome do  time???"
            />
          </label>
          <button
            type="submit"
            className="ml-5 bg-red-400 hover:bg-red-600 w-20 rounded-md"
          >
            Enviar
          </button>
